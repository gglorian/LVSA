...
> **Last verified:** 2026-06-11 — full step-time smoke sweep (every model × axis, world=2, 2× horizon, Dense vs LVSA-FlashInfer) on 2×A100; standalone `ulysses`+FlashInfer bit-identical to single-GPU at world=2. **2026-06-18:** Wan LVSA confirmed on the HTTP `serve.py` path under Ulysses (backend engages; hook delegates→backend), and HunyuanVideo confirmed to use the same `self.attn` backend seam as Wan (so its hook also delegates, not dense). **Also 2026-06-18:** Wan-**14B** serve+Ulysses engages sparse + completes (scale-up of the 1.3B serve confirm); HunyuanVideo plugin Ulysses @145f engages sparse + completes with no OOM (the OOM is only at 2× horizon). **2026-07-06 (vllm-omni 0.24.0rc1):** Cosmos + `--ulysses 2` **runs to completion, no `unpatchify` crash** — `gen_sp_gather` re-gathers the full grid before `unpatchify` (present since 0.22), so the earlier "Cosmos+Ulysses crashes upstream" claim was stale/incorrect and has been removed. **2026-07-07:** the Cosmos **backend seam-swap** (`LVSA_COSMOS3_BACKEND=1`, `cosmos3_backend.py`) installs LVSA on `Cosmos3CrossAttention`'s resolved attention backend ahead of that all-to-all, so it **re-engages sparse under Ulysses** (GPU-verified), matching Wan/HunyuanVideo; the forward-patch hook (`LVSA_COSMOS3_HOOK`) is removed. Update the tables when a new axis/model/path is validated (see [Updating this doc](#updating-this-doc)).
...
| Ulysses sequence-parallel (`ulysses_degree`) | sequence | ✅ (full grid via the framework all-to-all; Last-verified 2026-06-18, Wan2.1-1.3B @2× ulysses, backend. Cosmos's own backend seam-swap, `LVSA_COSMOS3_BACKEND=1`, also re-engages sparse under Ulysses — GPU-verified 2026-07-07) | ⚠️ inline path bails but **delegates to `self.attn` → backend → re-engages** for Wan/HunyuanVideo (verified Wan serve 2026-06-18) |
...
Which path each model works in. **Standalone CP** = `torchrun` context-parallel; **Plugin backend** = `LVSABackend` (offline + serve, all axes) — Cosmos uses its own backend seam-swap (`LVSA_COSMOS3_BACKEND=1`, `cosmos3_backend.py`) on `Cosmos3CrossAttention`, which engages the same way; **Plugin hook** = monkey-patch serve path used by **Wan / HunyuanVideo** (engages under TP; under sequence-parallel it *delegates to the backend*, which re-engages). Cosmos has no hook (the forward-patch hook was removed 2026-07-07 in favor of the backend seam, which also covers Ulysses). "all axes" = TP/CFG/PP/HSDP/Ulysses engage sparse (Ring always dense).
...
| Cosmos 3.0 | ⚠️ **single-GPU only** (diffusers processor swap; CP deferred) | ✅ **backend seam-swap** (`LVSA_COSMOS3_BACKEND=1` on `Cosmos3CrossAttention.attn`): TP/CFG/PP/HSDP **and Ulysses** (GPU-verified 2026-07-07 — engages sparse under Ulysses, `\|G\|=25 P=390 seq=19110` at single-GPU) · ✗ Ring (dense) | — (hook removed 2026-07-07; backend is the only path) |
| CogVideoX 5B | experimental (correctness only, no speedup) | — | — |

**Reading it:** Wan 2.1 (1.3B/14B) is the "works everywhere" case. Standalone CP is Wan-shaped (HV custom-only; TI2V-5B broken; A14B no win; Cosmos single-GPU only). The plugin **backend** is the most universal — every model engages under every axis except Ring, given the right `ref_lat`/`ppf`. **Cosmos** is split: *standalone* (`lvsa/cosmos3.py`) is a **diffusers processor swap → single-GPU only** (CP deferred). In the **plugin**, Cosmos now engages the same way as Wan/HunyuanVideo: a **backend seam-swap** (`LVSA_COSMOS3_BACKEND=1`, `cosmos3_backend.py`) installs LVSA on `Cosmos3CrossAttention`'s resolved attention backend (`self.attn`) at construction, ahead of the framework's Ulysses all-to-all — so it re-engages sparse on the reconstructed full grid under SP (GPU-verified 2026-07-07: Cosmos+Ulysses=2 engages sparse, matching Wan/HunyuanVideo's `LVSABackend` re-engagement). LVSA also engages sparse under TP/CFG/PP/HSDP and at single-GPU (`|G|=25, P=390, seq=19110`). Bottom line: Cosmos multi-GPU **runs everywhere and engages LVSA-sparse on every axis except Ring**; standalone still has no CP. Best speedup axes for the other models: **Ulysses + HSDP**; **TP is weakest** but is what unlocks the big models by sharding weights.
...
- **Cosmos standalone CP** — *standalone*: single-GPU only (diffusers processor swap, no CP; deferred). This is no longer a gap in the *plugin*: the backend seam-swap (`LVSA_COSMOS3_BACKEND=1`) engages sparse under Ulysses too (GPU-verified 2026-07-07), matching Wan/HunyuanVideo's `LVSABackend` re-engagement — only **Ring** stays dense for Cosmos (see the Ring bullet above).
...
- **Engaged:** the run log shows one of — `[LVSA] Geometry detected: … video_seq=N` (plugin Wan/HV/Cosmos backend) or `attended_per_frame=N/T` (standalone) — with `video_seq`/`local_seq == T_lat × P`.
